...
lv_obj_t *zmk_display_status_screen() {
    ...
    lv_obj_align(zmk_widget_battery_status_obj(&battery_status_widget), LV_ALIGN_BOTTOM_MID, 0, 0);

    // WPM widget - top left
    zmk_widget_wpm_init(&wpm_widget, screen);
    lv_obj_align(zmk_widget_wpm_obj(&wpm_widget), LV_ALIGN_TOP_LEFT, 0, 0);
